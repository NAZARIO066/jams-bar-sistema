...
def register_vendas_routes(app):
    ...
    def api_venda_direta():
        ...
        if forma == "Fiado":
            ...
            total_temp = 0
            for it in itens:
                p_row = db.execute("SELECT preco FROM produtos WHERE id=?", (it["produto_id"],)).fetchone()
                if p_row:
                    total_temp += float(it.get("quantidade", 0)) * p_row["preco"]
        ...
        resultado, status = processar_venda_direta(
            itens=itens, desconto=desconto, acrescimo=acrescimo, forma_pagamento=forma,
    ...
    def api_cancelar_venda(venda_id):
        ...
        if resultado.get("ok"):
            log_auditoria("CANCELAR_VENDA", f"Venda #{venda_id} cancelada")
        return jsonify(resultado), status
